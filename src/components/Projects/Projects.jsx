...
const CarouselItem = ({ project, index, total }) => {
  ...
  const thetaStart = Math.PI / 2 - sliceAngle / 2;
  const anglePosition = index * (Math.PI * 2 / total);

  return (
    <group rotation-y={anglePosition}>
      <mesh>
        <cylinderGeometry args={[radius, radius, height, 64, 1, true, thetaStart, sliceAngle]} />
        <meshBasicMaterial map={texture} side={THREE.DoubleSide} toneMapped={false} />
      </mesh>
      
      {/* Subtle border outline for the curve */}
      <mesh>
        <cylinderGeometry args={[radius + 0.01, radius + 0.01, height + 0.02, 64, 1, true, thetaStart, sliceAngle]} />
        <meshBasicMaterial color={project.color} wireframe transparent opacity={0.15} side={THREE.DoubleSide} />
      </mesh>
    </group>
  );
};
  ...
  return (
    <group ref={groupRef}>
      {projectData.map((project, i) => (
        <CarouselItem key={project.id} project={project} index={i} total={total} />
      ))}
    </group>
  );
};
      ...
      <AnimatePresence mode="wait">
        ...
        >
          ...
          <h3 
            ...
            style={{ fontFamily: 'var(--font-display)', color: 'white', textShadow: '0 10px 30px rgba(0,0,0,0.8)' }}
          >
...
export default function Projects() {
  ...
  const handleNav = (direction) => {
    ...
    const targetScroll = window.scrollY + rect.top + (nextIdx / (total - 1)) * scrollableHeight;
  ...
  return (
    <section ref={containerRef} id="projects" className="relative w-full h-[400vh] bg-[#020202]">
      <div className="sticky top-0 w-full h-screen overflow-hidden flex items-center justify-center">
        ...
        <div className="absolute top-8 right-8 flex gap-4 z-50 pointer-events-auto">
          <button 
            ...
            className="flex items-center justify-center w-12 h-12 rounded-full border border-white/20 text-white/60 hover:text-white hover:border-white/60 transition-all duration-300 disabled:opacity-20 backdrop-blur-md"
          >
            <ArrowLeft />
          </button>
          <button 
        ...
        <div className="absolute bottom-8 right-8 flex flex-col items-center gap-3 z-50 mix-blend-difference text-white">
          <span className="text-[10px] font-mono tracking-widest">{String(activeInt + 1).padStart(2, '0')}</span>
          <div className="w-[1px] h-12 bg-white/20 relative">
            <motion.div 
              className="absolute top-0 left-0 w-full bg-white origin-top"
              style={{ scaleY: useTransform(virtualIndex, [0, total - 1], [0, 1]) }}
            />
          </div>
          <span className="text-[10px] font-mono tracking-widest text-white/40">{String(total).padStart(2, '0')}</span>
        </div>
